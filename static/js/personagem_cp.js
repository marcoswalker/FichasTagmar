...
$('.btn_importJSON').click(function () {
    ...
    function fileRead(e) {
        ...
        $.post($SCRIPT_ROOT + "/uploadFoundry_cp/" + pers_id, {data: texto}, function () {
            location.reload();
        });
    }
});
...
function atributos_controll() {
    $('[name="data.atributos.INT"]').change(function () {
        att_atrib_pontos();
        att_pontos_hab();
        att_combate();
        att_tecnicas_table();
    });
    $('[name="data.atributos.AUR"]').change(function () {
        att_atrib_pontos();
        att_rm();
        att_karma();
        att_pontos_hab();
    ...
    $('[name="data.atributos.CAR"]').change(function () {
        att_atrib_pontos();
        att_pontos_hab();
        att_combate();
        att_tecnicas_table();
    });
    $('[name="data.atributos.FOR"]').change(function () {
        att_atrib_pontos();
    ...
    $('[name="data.atributos.FIS"]').change(function () {
        att_atrib_pontos();
        att_vb();
        att_rf();
        att_ef();
    ...
    $('[name="data.atributos.AGI"]').change(function () {
        att_atrib_pontos();
        att_pontos_hab();
        att_combate();
        att_tecnicas_table();
    ...
    $('[name="data.atributos.PER"]').change(function () {
        att_atrib_pontos();
        att_pontos_hab();
        att_combate();
        att_tecnicas_table();
...
function att_atrib_pontos() {
    ...
    let INT = parseInt($('[name="data.atributos.INT"]').val());
    let AUR = parseInt($('[name="data.atributos.AUR"]').val());
    let CAR = parseInt($('[name="data.atributos.CAR"]').val());
    let FOR = parseInt($('[name="data.atributos.FOR"]').val());
    let FIS = parseInt($('[name="data.atributos.FIS"]').val());
    let AGI = parseInt($('[name="data.atributos.AGI"]').val());
    let PER = parseInt($('[name="data.atributos.PER"]').val());

    let mod_racial_INT = parseInt($('[name="data.mod_racial.INT"]').val());
    let mod_racial_AUR = parseInt($('[name="data.mod_racial.AUR"]').val());
    let mod_racial_CAR = parseInt($('[name="data.mod_racial.CAR"]').val());
    let mod_racial_FOR = parseInt($('[name="data.mod_racial.FOR"]').val());
    let mod_racial_FIS = parseInt($('[name="data.mod_racial.FIS"]').val());
    let mod_racial_AGI = parseInt($('[name="data.mod_racial.AGI"]').val());
    let mod_racial_PER = parseInt($('[name="data.mod_racial.PER"]').val());

    if (raca.name === "Humano") pontos = 15;

    for (let x = 0; x <= estagio; x++) {
        if ((x % 2) != 0) pontos += 1;
    }

    if ((INT > mod_racial_INT) && (mod_racial_INT > 0)) { 
        pontos -= somaPontos(INT) - somaPontos(mod_racial_INT);
        g_int = somaPontos(INT) - somaPontos(mod_racial_INT);
    } else if ((INT > mod_racial_INT) && (mod_racial_INT < 0)) {
        pontos -= (somaPontos(INT) - mod_racial_INT);
        g_int = (somaPontos(INT) - mod_racial_INT);
    } else {
        pontos -= somaPontos(INT - mod_racial_INT);
        g_int = somaPontos(INT - mod_racial_INT);
    }

    if ((AUR > mod_racial_AUR) && (mod_racial_AUR > 0)) {
        pontos -= somaPontos(AUR) - somaPontos(mod_racial_AUR);
        g_aur = somaPontos(AUR) - somaPontos(mod_racial_AUR);
    } else if ((AUR > mod_racial_AUR) && (mod_racial_AUR < 0)) {
        pontos -= (somaPontos(AUR) - mod_racial_AUR);
        g_aur = (somaPontos(AUR) - mod_racial_AUR);
    } else {
        pontos -= somaPontos(AUR - mod_racial_AUR);
        g_aur = somaPontos(AUR - mod_racial_AUR);
    }

    if ((CAR > mod_racial_CAR) && (mod_racial_CAR > 0)) {
        pontos -= somaPontos(CAR) - somaPontos(mod_racial_CAR);
        g_car = somaPontos(CAR) - somaPontos(mod_racial_CAR);
    } else if ((CAR > mod_racial_CAR) && (mod_racial_CAR < 0)) {
        pontos -= (somaPontos(CAR) - mod_racial_CAR);
        g_car = (somaPontos(CAR) - mod_racial_CAR);
    } else {
        pontos -= somaPontos(CAR - mod_racial_CAR);
        g_car = somaPontos(CAR - mod_racial_CAR);
    }

    if ((FOR > mod_racial_FOR) && (mod_racial_FOR > 0)) {
        pontos -= somaPontos(FOR) - somaPontos(mod_racial_FOR);
        g_for = somaPontos(FOR) - somaPontos(mod_racial_FOR);
    } else if ((FOR > mod_racial_FOR) && (mod_racial_FOR < 0)) {
        pontos -= (somaPontos(FOR) - mod_racial_FOR);
        g_for = (somaPontos(FOR) - mod_racial_FOR);
    } else {
        pontos -= somaPontos(FOR - mod_racial_FOR);
        g_for = somaPontos(FOR - mod_racial_FOR);
    }

    if ((FIS > mod_racial_FIS) && (mod_racial_FIS > 0)) {
        pontos -= somaPontos(FIS) - somaPontos(mod_racial_FIS);
        g_fis = somaPontos(FIS) - somaPontos(mod_racial_FIS);
    } else if ((FIS > mod_racial_FIS) && (mod_racial_FIS < 0)) {
        pontos -= (somaPontos(FIS) - mod_racial_FIS);
        g_fis = (somaPontos(FIS) - mod_racial_FIS);
    } else {
        pontos -= somaPontos(FIS - mod_racial_FIS);
        g_fis = somaPontos(FIS - mod_racial_FIS);
    }

    if ((AGI > mod_racial_AGI) && (mod_racial_AGI > 0)) {
        pontos -= somaPontos(AGI) - somaPontos(mod_racial_AGI);
        g_agi = somaPontos(AGI) - somaPontos(mod_racial_AGI);
    } else if ((AGI > mod_racial_AGI) && (mod_racial_AGI < 0)) {
        pontos -= (somaPontos(AGI) - mod_racial_AGI);
        g_agi = (somaPontos(AGI) - mod_racial_AGI);
    } else {
        pontos -= somaPontos(AGI - mod_racial_AGI);
        g_agi = somaPontos(AGI - mod_racial_AGI);
    }

    if ((PER > mod_racial_PER) && (mod_racial_PER > 0)) {
        pontos -= somaPontos(PER) - somaPontos(mod_racial_PER);
        g_per = somaPontos(PER) - somaPontos(mod_racial_PER);
    } else if ((PER > mod_racial_PER) && (mod_racial_PER < 0)) {
        pontos -= (somaPontos(PER) - mod_racial_PER);
        g_per = (somaPontos(PER) - mod_racial_PER);
    } else {
        pontos -= somaPontos(PER - mod_racial_PER);
        g_per = somaPontos(PER - mod_racial_PER);
    }
    $('[name="data.carac_final.INT"]').val(pontos);
    $('[name="data.carac_sort.INT"]').val(g_int);
    $('[name="data.carac_sort.AUR"]').val(g_aur);
    $('[name="data.carac_sort.CAR"]').val(g_car);
    $('[name="data.carac_sort.FOR"]').val(g_for);
    $('[name="data.carac_sort.FIS"]').val(g_fis);
    $('[name="data.carac_sort.AGI"]').val(g_agi);
    $('[name="data.carac_sort.PER"]').val(g_per);
    $('[name="data.valor_teste.INT"]').val(INT*4);
    $('[name="data.valor_teste.AUR"]').val(AUR*4);
    $('[name="data.valor_teste.CAR"]').val(CAR*4);
    $('[name="data.valor_teste.FOR"]').val(FOR*4);
    $('[name="data.valor_teste.FIS"]').val(FIS*4);
    $('[name="data.valor_teste.AGI"]').val(AGI*4);
    $('[name="data.valor_teste.PER"]').val(PER*4);
}
